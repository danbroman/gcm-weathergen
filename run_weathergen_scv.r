...
#seasonal stat covariates
seas_dt = data.table(month = 1:12, seas = factor(rep(c('jfm', 'amj', 'jas', 'ond'), each = 3), levels = c('jfm', 'amj', 'jas', 'ond')))
weather_data_seas = weather_data_fl %>% left_join(seas_dt) %>% group_by(year, seas) %>% dplyr::summarise(prcp.seas = sum(prcp, na.rm = T), prcpocc.seas = sum(prcpocc, na.rm = T), tmin.seas = mean(tmin, na.rm = T), tmax.seas = mean(tmax, na.rm = T), rhmin.seas = mean(rhmin, na.rm = T), rhmax.seas = mean(rhmax, na.rm = T))
weather_data_seasvars = weather_data_fl %>% left_join(seas_dt) %>% dplyr::select(date, year, seas) %>% left_join(weather_data_seas) %>% arrange(date)

prcpocc.seas = as.numeric(as.matrix(dplyr::select(weather_data_seasvars, prcpocc.seas)))
prcpocc.seas1 = prcpocc.seas[-1]

prcp.seas = as.numeric(as.matrix(dplyr::select(weather_data_seasvars, prcp.seas)))
prcp.seas1 = prcp.seas[-1]

tmin.seas = as.numeric(as.matrix(dplyr::select(weather_data_seasvars, tmin.seas)))
tmin.seas1 = tmin.seas[-1]
	
tmax.seas = as.numeric(as.matrix(dplyr::select(weather_data_seasvars, tmax.seas)))
tmax.seas1 = tmax.seas[-1]

rhmin.seas = as.numeric(as.matrix(dplyr::select(weather_data_seasvars, rhmin.seas)))
rhmin.seas1 = rhmin.seas[-1]

rhmax.seas = as.numeric(as.matrix(dplyr::select(weather_data_seasvars, rhmax.seas)))
rhmax.seas1 = rhmax.seas[-1]
...
##fit models
#prcp occurrence
occ.dat = data.frame(prcp = as.factor(prcp.occ1), prcp.occn, daycos1, daysin1, daycos2x1, daysin3x1, daycos3x1, daysin2x1, daycosprcpocc = daycos1*prcp.occn, daysinprcpocc = daysin1*prcp.occn, prcpocc.seas1)

glm.occ = glm(prcp ~., data = occ.dat, family=binomial(), na.action=na.exclude)

#prcp intensity
int.dat = data.frame(prcp = prcp.int, daycos = daycos[prcp.occ == 1], daysin = daysin[prcp.occ == 1], daycos2x1 = daycos2x1[prcp.occ == 1], daysin2x1 = daysin2x1[prcp.occ == 1], daycos3x1 = daycos3x1[prcp.occ == 1], daysin3x1 = daysin3x1[prcp.occ == 1], prcp.seas[prcp.occ == 1])

glm.int = glm(prcp ~., data = int.dat, na.action=na.exclude)  

#temperature
tmin.dat = data.frame(tmin1, tminn, tmaxn, daycos1, daysin1, daycos2x1, daysin2x1, daysin3x1, daycos3x1, daysin4x1, daycos4x1, daysin5x1, daycos5x1, prcp.occ1, tmin.seas1)
tmax.dat = data.frame(tmax1, tmaxn, tmin1, daycos1, daysin1, daycos2x1, daysin2x1, daysin3x1, daycos3x1, daysin4x1, daycos4x1, daysin5x1, daycos5x1, prcp.occ1, tmax.seas1)
...
#relative humidity
rhmin.dat = data.frame(rhmin1, rhminn, rhmaxn, daycos1, daysin1, daycos2x1, daysin2x1, daysin3x1, daycos3x1, prcp.occ1, tmin1, tmax1, tminn, tmaxn, rhmin.seas1)
rhmax.dat = data.frame(rhmax1, rhmaxn, rhmin1, daycos1, daysin1, daycos2x1, daysin2x1, daysin3x1, daycos3x1, prcp.occ1, tmin1, tmax1, tminn, tmaxn, rhmax.seas1)
...
if(pred_flag == T){ #simulation loop
	...
	wgen_sims = foreach(icount(nsim)) %dopar% { #simulation loop
		...
		for (k in 2:nod){
			...
			#prcp occurrence
			covocc = as.numeric(c(1, prcp.occ.sim[k - 1], daycos[k], daysin[k], daycos2x[k], daysin2x[k], daycos3x[k], daysin3x[k], daycos[k]*prcp.occ.sim[k - 1], daysin[k]*prcp.occ.sim[k - 1], prcpocc.seas[k]))
			...
			#temperature
			covtmin  = c(1, tmin.sim[k-1], tmax.sim[k-1], daycos[k], daysin[k], daycos2x[k], daysin2x[k], daycos3x[k], daysin3x[k], daycos4x[k], daysin4x[k], daycos5x[k], daysin5x[k], prcp.occ.sim[k], tmin.seas[k])
			tmin.sim[k] = coeftmin %*% covtmin + rnorm(1, 0, sigmatmin)
			covtmax  = c(1, tmax.sim[k-1], tmin.sim[k], daycos[k], daysin[k],daycos2x[k],daysin2x[k], daycos3x[k], daysin3x[k], daycos4x[k], daysin4x[k], daycos5x[k], daysin5x[k], prcp.occ.sim[k], tmax.seas[k])
			...
			#relative humidity
			covrhmin = as.numeric(c(1, rhmin.sim[k-1], rhmax.sim[k-1], daycos[k], daysin[k], daycos2x[k],daysin2x[k], daycos3x[k], daysin3x[k], prcp.occ.sim[k], tmin.sim[k], tmax.sim[k], tmin.sim[k-1], tmax.sim[k-1], rhmin.seas[k]))	
				rhmin.sim[k] = inv.logit(coefrhmin %*% covrhmin)
			covrhmax = as.numeric(c(1, rhmax.sim[k-1], rhmin.sim[k], daycos[k], daysin[k], daycos2x[k],daysin2x[k], daycos3x[k], daysin3x[k], prcp.occ.sim[k], tmin.sim[k], tmax.sim[k], tmin.sim[k-1], tmax.sim[k-1], rhmax.seas[k]))
		...
		#prcp intensity
		covint = cbind(1, daycos[which(prcp.occ.sim == 1)], daysin[which(prcp.occ.sim == 1)], daycos2x[which(prcp.occ.sim == 1)], daysin2x[which(prcp.occ.sim == 1)], daycos3x[which(prcp.occ.sim == 1)], daysin3x[which(prcp.occ.sim == 1)], prcp.seas[which(prcp.occ.sim == 1)])
